...
def Delay(value):
	while not(value <= 1):
		value -= 0.123
		print(f'''{trang}[{hong}TONY{tim}] [{hong}DELAY{trang}] [{hong}{str(value)[0:5]}{trang}] [{vang}█    {trang}]''', '               ', end = '\r')
		sleep(0.010)
		print(f'''{trang}[{hong}TONY{tim}] [{hong}DELAY{trang}] [{hong}{str(value)[0:5]}{trang}] [ {vang}█   {trang}]''', '               ', end = '\r')
		sleep(0.010)
		print(f'''{trang}[{hong}TONY{tim}] [{hong}DELAY{trang}] [{hong}{str(value)[0:5]}{trang}] [  {vang}█  {trang}]''', '               ', end = '\r')
...
def Main():  
    ...
    while True:
        if os.path.exists('cookiefb.txt'):
		    ...
		    # Hiệu ứng tải
                ...
                print(f'{thanh} {red}Mẹ mày mù à?Nhập lại')
                thanhngang(70)
                continue
    ...
    while True:
        if len(listck) == 0:
            print(f'{red}Đã Xoá Tất Cả Cookie,Vui Lòng Nhập Lại Cookie Mới')
            listck = Nhap_Cookie()
